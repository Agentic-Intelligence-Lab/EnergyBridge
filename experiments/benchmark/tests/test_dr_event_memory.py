...
def test_dr_event_memory_does_not_cross_entity_scope() -> None:
    historical_result = {
        "method": "eb_rule_milp",
        "weather": "Germany",
        "sim_days": 1,
        ...
        "vpp_event_log": [
            {
                ...
                "end_h": 19.0,
                "actual_kwh": 1.0,
                "counterfactual_baseline_kwh": 5.0,
                "counterfactual_actual_shed_kwh": 4.0,
                "capacity_window_summary": {"recommended_bid_energy_kwh": 2.0},
                "score": 4,
            }
        ],
    }
    ...
    estimated = apply_dr_memory_capacity_estimate(
        future_result,
        memory,
        metadata={"household_id": "household_b", "method": "eb_rule_milp", "city": "Germany"},
        top_k=3,
    )

    event_estimate = estimated["vpp_event_log"][0]["historical_dr_memory_capacity_estimate"]
    assert event_estimate["status"] == "no_matching_history"
    assert event_estimate["retrieval_count"] == 0
    assert event_estimate["reported_capacity_kwh"] == 2.0
