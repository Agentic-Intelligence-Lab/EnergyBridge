...
# ------------------------------------------------------------------
# Fix 'agents' package name conflict BEFORE any HEMA import
# ------------------------------------------------------------------
...
class EnergyBridgeToHEMA:
    ...
    def ensure_device_config(self, appliance_config: Dict[str, Any], eplus_state: Dict[str, Any]):
        ...
        devices["hvac"] = {
            ...
            "settings": {
                ...
                "default_temperature_f": 74,
                "away_mode_temp_f": 85,
            },
            "control_actions": [
                {"action": "set_temperature", "params": ["temperature"], "description": "Set target temperature in °F"},
                {"action": "set_mode", "params": ["mode"], "description": "Set mode: cool/heat/auto/off"},
            ...
            "energy_info": {"rated_power_kw": 3.5},
        }
        ...
        if appliance_config.get("dishwasher", {}).get("present"):
            ...
            devices["dishwasher"] = {
                ...
                "energy_info": {"rated_power_kw": cfg.get("power_kw", 1.5)},
            }
        ...
        if appliance_config.get("water_heater", {}).get("present"):
            ...
            preheat_temp_c = cfg.get("pre_heat_temp_c", 65.0)
            devices["water_heater"] = {
                ...
                "current_state": {
                    "power": "on",
                    "mode": "auto",
                    "current_water_temp_f": round(_c_to_f(55.0), 1),
                    "target_water_temp_f": round(_c_to_f(preheat_temp_c), 1),
                    "scheduled_start_time": None,
                },
                "settings": {
                    "temperature_range_f": {"min": 100, "max": 170},
                    "modes": ["auto", "vacation", "efficiency"],
                },
                "control_actions": [
        ...
        if appliance_config.get("ev", {}).get("present"):
            ...
            devices["ev_charger"] = {
                ...
                "current_state": {
                    ...
                    "vehicle_battery_percent": 50,
                    "charge_limit_percent": 80,
                    "max_charge_rate_kw": cfg.get("power_kw", cfg.get("charger_kw", 7.0)),
                },
                "settings": {"modes": ["smart", "delay", "normal"]},
                "control_actions": [
                    {"action": "set_mode", "params": ["mode"], "description": "Set charging mode: smart/delay/normal"},
                ...
                "energy_info": {"rated_power_kw": cfg.get("power_kw", cfg.get("charger_kw", 7.0))},
            }
    ...
    def build_query(
        ...
        # Scoring weights (quantitative priorities)
        scoring_weights = preferences.get("scoring_weights", {})
        vpp_override_prob = preferences.get("vpp_override_prob", 0.5)
        ...
        # ═══════════════════════════════════════════════════════
        # SECTION 1: User Profile Core (from llm_prompts.system_prompt)
        # ═══════════════════════════════════════════════════════
        ...
        lines += [
            ...
            "",
            "[User Priority Weights — USE THESE FOR TRADE-OFF DECISIONS]",
        ]
        ...
        if scoring_weights:
            ...
            lines += [
                ...
                f"  → When these conflict, weight by these ratios. Comfort={comfort_w:.2f} is the dominant constraint.",
            ]
        else:
            lines += ["  (No explicit weights provided; use moderate balanced approach.)"]

        lines += [
            f"  VPP override probability: {float(vpp_override_prob):.2f} "
            f"({'user likely rejects VPP' if vpp_override_prob > 0.5 else 'user likely accepts VPP'})",
            "",
        ]
        ...
        if ac_cfg:
            ...
            if ac_mode == "cooling":
                ...
                lines.append("  Current season: HEATING. Setpoint adjustments should LOWER temperature to save energy.")
        ...
        if vpp_event:
            ...
            # Tag-based VPP strategy hints
            if tags.get("comfort") == "temp_sensitive":
                vpp_lines += [
                    "   → User is TEMPERATURE-SENSITIVE: minimize setpoint changes during VPP.",
                    "   → Prefer appliance shifting over thermostat adjustment.",
                ]
            if tags.get("control") == "confirm_required":
                vpp_lines += [
                    "   → User requires CONFIRMATION: any VPP action must be justified.",
                ]
            if tags.get("price") == "low_incentive":
                vpp_lines += [
                    "   → User is NOT motivated by small savings: only participate if benefit is substantial.",
                ]
            if tags.get("grid_value") == "low_value":
                vpp_lines += [
                    "   → User explicitly NOT suitable as DR target: focus on comfort preservation.",
                ]
        ...
        if not any((appliances.get(n, {}).get("present") for n in
                    ("washer", "dishwasher", "dryer", "water_heater", "ev"))):
            lines += ["- (no controllable appliances configured)"]
                    ...
                    if name == "control_device":
                        if dev in ("hvac", "thermostat", "ac", "heat", "heater"):
                            if action in ("set_temperature", "set_temp", "temperature"):
                                try:
                                    setpoint_f = float(str(value).replace("°", "").replace("F", "").replace("f", "").replace("C", "").replace("c", "").strip())
                                except Exception:
                                    pass
                            elif action == "away":
                                setpoint_f = 78.0

                        elif dev in ("washing_machine", "washer", "laundry"):
                            if action in ("set_schedule", "schedule"):
                                washer_start = _parse_hod(value or time_str)

                        elif dev in ("dishwasher",):
                            if action in ("set_schedule", "schedule"):
                                dishwasher_start = _parse_hod(value or time_str)

                        elif dev in ("clothes_dryer", "dryer"):
                            if action in ("set_schedule", "schedule"):
                                dryer_start = _parse_hod(value or time_str)

                        elif dev in ("water_heater", "hot_water", "boiler"):
                            if action in ("set_temperature", "set_temp", "temperature"):
                                try:
                                    wh_temp_f = float(str(value).replace("°", "").replace("F", "").replace("f", "").replace("C", "").replace("c", "").strip())
                                    wh_preheat = True
                                except Exception:
                                    pass
                            elif action in ("set_schedule", "schedule"):
                                wh_start = _parse_hod(value or time_str)
                            ...
                            if action in ("set_mode", "mode", "change_mode"):
                                ...
                                if mode in ("smart", "delay", "normal"):
                                    ev_mode = mode
                            elif action in ("start_charging", "begin_charging"):
                                ev_mode = "normal"
                            elif action in ("stop_charging", "end_charging"):
                                ev_mode = "delay"

                    elif name == "schedule_device_action":
                        if dev in ("washing_machine", "washer", "laundry"):
                            ...
                            wh_end = (wh_start + 4.0) % 24.0 if wh_start is not None else None
                            wh_preheat = True
                        elif dev in ("ev_charger", "ev", "electric_vehicle"):
                            ev_mode = "delay"

        # Fallbacks from final HEMA device state
        config = load_device_config()
        devices = config.get("devices", {})

        hvac = devices.get("hvac", {})
        if setpoint_f is None:
            setpoint_f = float(hvac.get("current_state", {}).get("target_temperature_f", 77.0))

        wm = devices.get("washing_machine", {})
        if washer_start is None:
            washer_start = _parse_hod(wm.get("current_state", {}).get("scheduled_start_time"))

        dw = devices.get("dishwasher", {})
        if dishwasher_start is None:
            dishwasher_start = _parse_hod(dw.get("current_state", {}).get("scheduled_start_time"))

        cd = devices.get("clothes_dryer", {})
        if dryer_start is None:
            dryer_start = _parse_hod(cd.get("current_state", {}).get("scheduled_start_time"))

        wh = devices.get("water_heater", {})
        if wh_temp_f is None:
            wh_temp_f = float(wh.get("current_state", {}).get("target_water_temp_f", 149.0))
            if wh_temp_f > 100:
                wh_preheat = True
        if wh_start is None:
            wh_start = _parse_hod(wh.get("current_state", {}).get("scheduled_start_time"))

        ev = devices.get("ev_charger", {})
        ev_state = ev.get("current_state", {})
        if ev_state.get("mode") in ("smart", "delay", "normal"):
            ev_mode = ev_state["mode"]
