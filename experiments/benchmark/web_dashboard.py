...
FRONTEND_INDEX_PATH = PROJECT_ROOT / "energybridge" / "frontend" / "agent_console.html"
JOBS: dict[str, dict] = {}
...
def _validate_run_command(command: str) -> list[str]:
    ...
    if "--method" in argv:
        ...
        if str(method).strip().lower() not in {"energybridge", "agent"}:
            raise ValueError("The dashboard only starts EnergyBridge Agent runs.")
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == "/":
            self._send_text(_dashboard_index_html(), "text/html; charset=utf-8")
            return
