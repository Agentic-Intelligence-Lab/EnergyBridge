...
from .weather_shift import features as _weather_features
...
def estimate_event_capacity_from_memory(
    ...
    *,
    ...
    method = _method(metadata, result)
    start_date = str(metadata.get("start_date") or result.get("start_date") or "")
    target_weather = _event_weather(city, start_date)
    same_group_candidates = [
        record
        for record in memory.get("events") or []
        if str(record.get("entity_id") or "") == entity_id
        and str(record.get("city") or "").lower() == city.lower()
        and not record.get("data_quality_excluded")
    ]
    weather_stats = _weather_group_stats(same_group_candidates, target_weather)
    scored = []
    for record in memory.get("events") or []:
        if str(record.get("entity_id") or "") != entity_id:
            continue
        if record.get("data_quality_excluded"):
            continue
        score = _similarity(
            record,
            entity_id=entity_id,
...
def _similarity(
    ...
    *,
    ...
    target_weather: Mapping[str, Any] | None = None,
    weather_stats: Mapping[str, Any] | None = None,
) -> float:
    score = 0.0
    ...
    if rec_hour is not None:
        ...
        score += _W_HOUR * max(0.0, 1.0 - diff / 12.0)
    rec_duration = _float_or_none(record.get("duration_h"))
    if rec_duration is not None:
        score += _W_DURATION * max(0.0, 1.0 - abs(duration_h - rec_duration) / max(duration_h, rec_duration, 1.0))
    ...
    if target_baseline_kwh is not None and rec_baseline is not None and max(target_baseline_kwh, rec_baseline) > EPS:
        ...
        score += _W_BASELINE * max(0.0, ratio)
    ...
    if target_day is not None and rec_day is not None:
        ...
        score += _W_DAY_PROXIMITY * max(0.0, 1.0 - min(day_diff, 7) / 7.0)
    score += _WEATHER_WEIGHT * _weather_similarity(record.get("weather_features"), target_weather, weather_stats)
    return score
...
def _daily_weather_cache(city: str) -> dict[str, dict[str, float]]:
    key = city.strip().lower()
    if key not in _WEATHER_DAILY_CACHE:
        try:
            _WEATHER_DAILY_CACHE[key] = _weather_features.daily_features_for_city(city)
        except (ValueError, FileNotFoundError, OSError):
            _WEATHER_DAILY_CACHE[key] = {}
    return _WEATHER_DAILY_CACHE[key]


def _event_weather(city: str, start_date: str) -> dict[str, float] | None:
    if not city or not start_date:
        return None
    daily = _daily_weather_cache(city)
    if not daily:
        return None
    return _weather_features.event_weather_features(city, start_date, daily_cache=daily)


def _weather_vector(weather: Mapping[str, Any] | None) -> list[float] | None:
    if not weather:
        return None
    try:
        return [float(weather[name]) for name in _weather_features.FEATURE_NAMES]
    except (KeyError, TypeError, ValueError):
        return None


def _weather_group_stats(
    records: list[Mapping[str, Any]],
    target_weather: Mapping[str, Any] | None,
) -> dict[str, Any] | None:
    """Per-(household, city) z-score stats + KNN-style Gaussian bandwidth.

    Same normalization/bandwidth convention as the weather-shift KNN(k=15)
    fallback (median distance from the query to the group, min 1e-3).
    """
    target_vec = _weather_vector(target_weather)
    if target_vec is None:
        return None
    vectors = [v for v in (_weather_vector(r.get("weather_features")) for r in records) if v is not None]
    if len(vectors) < _WEATHER_MIN_GROUP_SIZE:
        return None
    dims = len(_weather_features.FEATURE_NAMES)
    n = len(vectors)
    means = [sum(v[i] for v in vectors) / n for i in range(dims)]
    stds = [max(1e-6, (sum((v[i] - means[i]) ** 2 for v in vectors) / n) ** 0.5) for i in range(dims)]
    target_z = [(target_vec[i] - means[i]) / stds[i] for i in range(dims)]
    distances = []
    for v in vectors:
        vz = [(v[i] - means[i]) / stds[i] for i in range(dims)]
        distances.append(math.sqrt(sum((target_z[i] - vz[i]) ** 2 for i in range(dims))))
    bandwidth = max(1e-3, median(distances))
    return {"mean": means, "std": stds, "bandwidth": bandwidth}


def _weather_similarity(
    record_weather: Mapping[str, Any] | None,
    target_weather: Mapping[str, Any] | None,
    weather_stats: Mapping[str, Any] | None,
) -> float:
    if not weather_stats:
        return 0.0
    target_vec = _weather_vector(target_weather)
    record_vec = _weather_vector(record_weather)
    if target_vec is None or record_vec is None:
        return 0.0
    means = weather_stats["mean"]
    stds = weather_stats["std"]
    bandwidth = weather_stats["bandwidth"]
    target_z = [(target_vec[i] - means[i]) / stds[i] for i in range(len(means))]
    record_z = [(record_vec[i] - means[i]) / stds[i] for i in range(len(means))]
    dist = math.sqrt(sum((target_z[i] - record_z[i]) ** 2 for i in range(len(means))))
